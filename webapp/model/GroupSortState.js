sap.ui.define([
	...
	return BaseObject.extend("com.bhaskar.in.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias com.bhaskar.in.model.GroupSortState
		 */
		...
		/**
		 * Sorts by ProductName, or by UnitsInStock
		 *
		...
		/**
		 * Groups by UnitsInStock, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function(sKey) {
			...
			if (sKey === "UnitsInStock") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "UnitsInStock");

				aSorters.push(
					new Sorter("UnitsInStock", false,
